...
export class ImportantTasksComponent implements OnInit, OnDestroy {
  tasks: Task[] = [];
  isLoading = false;
  errorMessage = '';
  search = '';
  ...
  get filteredTasks(): Task[] {
    // Solo tareas importantes
    const importantTasks = this.tasks.filter(t => t.important === true);
    
    const term = this.search.trim().toLowerCase();
    if (!term) return importantTasks;

    return importantTasks.filter(t =>
  ...
  loadTasks(): void {
    ...
    forkJoin([created$, assigned$]).subscribe({
      next: ([created, assigned]) => {
        ...
        // Guardar todas las tareas, el filtro de importantes se hace en filteredTasks
  ...
  handleTaskAdded(taskData: TaskFormData): void {
    ...
    const taskRequest: TaskRequest = {
      ...
      important: true // Las tareas creadas aquí son importantes por defecto
    };
